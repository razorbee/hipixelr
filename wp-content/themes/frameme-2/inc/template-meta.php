 ...
 * @return array
 */
function frameme_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! frameme_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$defaults = apply_filters( 'frameme_default_args_share_buttons', array(
		...
		'pinterest' => array(
			...
			'name'      => esc_html__( 'Pinterest', 'frameme' ),
	...
	$default_config = apply_filters( 'frameme_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'frameme_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'frameme' ) . $network['name'],
 ...
 * @return string
 */
function frameme_get_single_share_buttons() {
	ob_start();
