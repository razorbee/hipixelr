...
/**
 * Class FrameMe_News_Smart_Box_Widget
 */
class FrameMe_News_Smart_Box_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_id          = 'frameme_widget_news_smart_box';
		$this->widget_name        = esc_html__( 'News Smart Box', 'frameme' );
		$this->widget_description = esc_html__( 'News Smart Box', 'frameme' );
		$this->utility            = frameme_utility()->utility;
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'News Smart Box', 'frameme' ),
			...
			'layout_type' => array(
				...
				'options' => array(
					'layout_type_1' => array(
						'label'   => esc_html__( 'Layout type 1', 'frameme' ),
						'img_src' => FRAMEME_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-1.svg',
					),
					'layout_type_2' => array(
						'label'   => esc_html__( 'Layout type 2', 'frameme' ),
						'img_src' => FRAMEME_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-2.svg',
					),
					'layout_type_3' => array(
						'label'   => esc_html__( 'Layout type 3', 'frameme' ),
						'img_src' => FRAMEME_THEME_URI . '/assets/images/admin/widgets/news-smart-box/layout-3.svg',
					),
				),
				'label' => esc_html__( 'Choose layout type', 'frameme' ),
			),
			'terms_type' => array(
				'type'    => 'radio',
				'value'   => 'category',
			...
			'categories' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'category', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary categories', 'frameme' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select categories', 'frameme' ),
				'master'           => 'terms_type_category',
			),
			'current_tag' => array(
				'type'             => 'select',
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select tag', 'frameme' ),
				'placeholder'      => esc_html__( 'Select tag', 'frameme' ),
			...
			'tags' => array(
				...
				'options_callback' => array( $this, 'get_terms_list', array(
					'post_tag', 'slug', ) ),
				'options'          => false,
				'label'            => esc_html__( 'Select secondary tags', 'frameme' ),
				'multiple'         => true,
			...
			'current_title_visibility' => array(
				...
				'label' => esc_html__( 'Display current navigation title', 'frameme' ),
			),
			'date_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display date', 'frameme' ),
			),
			'author_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'frameme' ),
			),
			'comment_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'frameme' ),
			),
			'category_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display categories', 'frameme' ),
			),
			'tags_visibility' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display tags', 'frameme' ),
			),
		...
		add_filter( 'frameme_theme_script_variables', array( $this, 'add_new_smart_box_nonce_var' ) );
	...
	public function add_new_smart_box_nonce_var( $vars = array() ) {
		$vars['new_smart_box_nonce'] = wp_create_nonce( 'frameme-new-smart-box-nonce' );
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'frameme_news_smart_box_default_query_args', array(
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
			'posts_per_page' => 10,
	...
	public function get_smart_box_loop( $posts_query ) {
		...
		if ( ! $posts_query->have_posts() ) {
			return '<div class="container"><h4>' . esc_html__( 'Posts not found', 'frameme' ) . '</h4></div>';
		}
		...
		while ( $posts_query->have_posts() ) : $posts_query->the_post();
			...
			$image_size      = 'frameme-thumb-560-315';
			$title_tag       = 'h4';
			...
			switch ( $this->instance['layout_type'] ) {
				case 'layout_type_1':
					if ( 1 <= $posts_query->current_post ) {
						...
						$image_size      = 'frameme-thumb-260-147';
						$title_tag       = 'h5';
					} else {
	...
	public function new_smart_box_instance(){
		if ( ! isset( $_GET['nonce'] ) || ! wp_verify_nonce( $_GET['nonce'], 'frameme-new-smart-box-nonce' ) ) {
			exit();
		}
...
add_action( 'widgets_init', 'frameme_register_news_smart_box_widgets' );
/**
 * Register news-smart-box widget.
 */
function frameme_register_news_smart_box_widgets() {
	register_widget( 'FrameMe_News_Smart_Box_Widget' );
}
