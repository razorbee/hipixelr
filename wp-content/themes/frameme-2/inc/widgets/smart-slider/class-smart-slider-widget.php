...
if ( ! class_exists( 'FrameMe_Smart_Slider_Widget' ) ) {

	/**
	 * Class FrameMe_Smart_Slider_Widget.
	 */
	class FrameMe_Smart_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display smart slider on your site.', 'frameme' );
			$this->widget_id          = 'frameme_widget_smart_slider';
			$this->widget_name        = esc_html__( 'Smart Slider', 'frameme' );
			...
			$this->settings           = array(
				...
				'title_trim_words' => array(
					...
					'min_value'  => 1,
					'step_value' => 1,
					'label'      => esc_html__( 'Title words trimmed count', 'frameme' ),
				),
				'trim_words' => array(
					'type'       => 'slider',
					'value'      => 15,
				...
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width', 'frameme' ),
				),
				'height' => array(
					'type'  => 'text',
					'value' => '800',
					'label' => esc_html__( 'Slider height', 'frameme' ),
				),
				'orientation' => array(
					'type'    => 'select',
					'size'    => 1,
					...
					'label'   => esc_html__( 'Slider orientation', 'frameme' ),
				),
				'slide_distance' => array(
					...
					'max_value' => 100,
					'min_value' => 0,
					'label'     => esc_html__( 'Slide distance(px)', 'frameme' ),
				),
				'slide_duration' => array(
					...
					'min_value'  => 100,
					'step_value' => 100,
					'label'      => esc_html__( 'Slide duration(ms)', 'frameme' ),
				),
				'slide_fade' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use fade effect?', 'frameme' ),
				),
				'navigation' => array(
					'type'   => 'switcher',
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Use navigation?', 'frameme' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'frameme' ),
						'false_toggle' => esc_html__( 'Off', 'frameme' ),
				...
				'fade_navigation' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Indicates whether the arrows will fade in only on hover', 'frameme' ),
				...
				'pagination' => array(
					...
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use pagination?', 'frameme' ),
				),
				'autoplay' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use autoplay?', 'frameme' ),
				),
				'fullScreen' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Display fullScreen button?', 'frameme' ),
				),
				'shuffle' => array(
					'type'  => 'switcher',
					'value' => 'false',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Indicates if the slides will be shuffled', 'frameme' ),
				),
				'loop' => array(
					'type'  => 'switcher',
					'value' => 'true',
					'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label' => esc_html__( 'Use infinite scrolling?', 'frameme' ),
				),
				'thumbnails' => array(
					'type'   => 'switcher',
					'value'  => 'false',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails?', 'frameme' ),
					'toggle' => array(
						'true_toggle'  => esc_html__( 'On', 'frameme' ),
						'false_toggle' => esc_html__( 'Off', 'frameme' ),
				...
				'thumbnails_position' => array(
					...
					'options' => array(
						...
						'bottom' => esc_html__( 'Bottom', 'frameme' ),
						'right'  => esc_html__( 'Right', 'frameme' ),
						'left'   => esc_html__( 'Left', 'frameme' ),
					),
					'label'   => esc_html__( 'Sets the position of the thumbnail scroller', 'frameme' ),
				...
				'thumbnails_arrows' => array(
					...
					'value'  => 'true',
					'style'  => ( wp_is_mobile() ) ? 'normal' : 'small',
					'label'  => esc_html__( 'Display thumbnails arrows?', 'frameme' ),
					'master' => 'thumbnails_attr',
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( $posts_query ) {
				...
				$html = '<h4>' . esc_html__( 'Posts for slider not found', 'frameme' ) . '</h4>';
			}
		...
		public function render_slider( $posts_query ) {
			...
			$html = '<div class="frameme-smartslider smart-slider__instance invert" ' . $slider_html_attr . '>';
		...
		public function get_query_slider_items( $query_args = array() ) {
			...
			$defaults_query_args = apply_filters( 'frameme_smart_slider_default_query_args', array(
				'post_type'           => 'post',
				'orderby'             => 'date',
				'order'               => 'DESC',
				'posts_per_page'      => - 1,
		...
		public function get_slider_loop( $posts_query ) {
			 ...
			 * @var  array
			 */
			$title_settings = apply_filters( 'frameme_smart_slider_title_settings', array(
				'width'           => '100%%',
				'horizontal'      => '0%%',
				'vertical'        => '38%%',
			 ...
			 * @var  array
			 */
			$content_settings = apply_filters( 'frameme_smart_slider_content_settings', array(
				'width'           => '100%%',
				'horizontal'      => '0%%',
				'vertical'        => '51%%',
				'show_transition' => 'left',
			 ...
			 * @var  array
			 */
			$more_settings = apply_filters( 'frameme_smart_slider_more_button_settings', array(
				'width'           => '100%%',
				'horizontal'      => '0%%',
				'vertical'        => '58%%',
				'show_transition' => 'left',
		...
		public function get_slider_thumbnails( $posts_query ) {
			...
			while ( $posts_query->have_posts() ) : $posts_query->the_post();
				$html .= '<div class="sp-thumbnail">';
					$html .= $this->utility->media->get_image( array(
						'size'        => 'frameme-slider-thumb',
						'mobile_size' => 'frameme-slider-thumb',
	...
	add_action( 'widgets_init', 'frameme_register_smart_slider_widgets' );
	/**
	 * Register Smart slider widget.
	 */
	function frameme_register_smart_slider_widgets() {
		register_widget( 'FrameMe_Smart_Slider_Widget' );
	}
}
