...
if ( ! class_exists( 'FrameMe_Taxonomy_Tiles_Widget' ) ) {

	/**
	 * Taxonomy Tiles Widget
	 */
	class FrameMe_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Taxonomy Tiles', 'frameme' );
			$this->widget_description = esc_html__( 'This widget displays images from taxonomy.', 'frameme' );
			$this->widget_id          = 'frameme-widget-taxonomy-tiles';
			...
			$this->settings           = array(
				'title' => array(
					...
					'label' => esc_html__( 'Widget title', 'frameme' ),
				),
				'terms_type' => array(
					'type'    => 'radio',
					'value'   => 'category',
				...
				'category' => array(
					...
					'label'            => esc_html__( 'Select category to show', 'frameme' ),
				...
				'post_tag' => array(
					...
					'label'            => esc_html__( 'Select tags to show', 'frameme' ),
				...
				'description_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Description words length ( Set 0 to hide description. )', 'frameme' ),
				),
				'show_post_count' => array(
					...
					'options' => array(
						'show_post_count_check' => esc_html__( 'Show post count', 'frameme' ),
					),
				),
				'layout_type' => array(
					'type'    => 'radio',
					...
					'options' => array(
						'grid'  => array(
							'label' => esc_html__( 'Grid', 'frameme' ),
							'slave' => 'layout_type_grid',
						),
						'tiles' => array(
							'label' => esc_html__( 'Tiles', 'frameme' ),
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number ( layout type grid )', 'frameme' ),
				...
				'tiles_type' => array(
					...
					'options' => array(
						'tiles-layout-1' => array(
							'label' => esc_html__( 'Layout 1', 'frameme' ),
						),
						'tiles-layout-2' => array(
							'label' => esc_html__( 'Layout 2', 'frameme' ),
						),
					),
				...
				'items_padding' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Items padding ( size in pixels )', 'frameme' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				...
				$grid_class_array = apply_filters( 'frameme_taxonomy_tiles_widget_grid_class', array(
				...
				echo apply_filters( 'frameme_taxonomy_tiles_widget_before',
				...
				foreach ( $terms as $term_key => $term ) {
					...
					$count = $this->utility->meta_data->get_post_count_in_term( array(
						...
						'sufix'   => _n_noop( '%s post', '%s posts', 'frameme' ),
					...
					if ( 'grid' === $layout_type ) {
						...
						$bg_image_url = $this->utility->media->get_image( array(
							'html'        => '%3$s',
							'size'        => 'frameme-thumb-760-571',
							'mobile_size' => 'frameme-thumb-760-571',
						), 'term', $term->term_id );
				...
				echo apply_filters( 'frameme_taxonomy_tiles_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'frameme_register_taxonomy_tiles_widget' );
	/**
	 * Register taxonomy tiles widget.
	 */
	function frameme_register_taxonomy_tiles_widget() {
		register_widget( 'FrameMe_Taxonomy_Tiles_Widget' );
	}
}
