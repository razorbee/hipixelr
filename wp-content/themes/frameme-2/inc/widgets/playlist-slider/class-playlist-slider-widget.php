...
if ( ! class_exists( 'FrameMe_Playlist_Slider_Widget' ) ) {

	/**
	 * Class FrameMe_Playlist_Slider_Widget.
	 */
	class FrameMe_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'frameme' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'frameme' );
			$this->widget_id          = apply_filters( 'frameme_playlist_slider_widget_ID', 'frameme_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'frameme_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = frameme_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'frameme' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'frameme' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'frameme' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '525',
					'label' => esc_html__( 'Slider height ( px, rem )', 'frameme' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'frameme' ),
						true  => esc_html__( 'Buttons', 'frameme' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'frameme' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'frameme' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'frameme' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'frameme' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'frameme' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'frameme' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'frameme' ),
						'author'   => esc_html__( 'Author', 'frameme' ),
						'comments' => esc_html__( 'Comment count', 'frameme' ),
						'category' => esc_html__( 'Category', 'frameme' ),
						'post_tag' => esc_html__( 'Tag', 'frameme' ),
					),
					'label' => esc_html__( 'Display post meta data', 'frameme' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'frameme' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'frameme_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			...
			foreach ( $this->posts as $post ) {
				...
				frameme_theme()->dynamic_css->add_style(
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'frameme_playlist_buttons', false ),
...
add_action( 'widgets_init', 'frameme_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function frameme_register_playlist_slider_widget() {
	register_widget( 'FrameMe_Playlist_Slider_Widget' );
}
