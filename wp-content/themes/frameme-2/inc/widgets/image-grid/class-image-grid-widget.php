 ...
 * @package FrameMe
 */
if ( ! class_exists( 'FrameMe_Image_Grid_Widget' ) ) {

	/**
	 * Image Grid Widget
	 */
	class FrameMe_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Image Grid', 'frameme' );
			$this->widget_description = esc_html__( 'This widget displays images from post.', 'frameme' );
			...
			$this->settings           = array(
				...
				'category_name' => array(
					...
					'label'            => esc_html__( 'Select categories to show', 'frameme' ),
				...
				'post_sort' => array(
					...
					'value'   => 'date',
					'options' => array(
						'date'          => esc_html__( 'Publish Date', 'frameme' ),
						'title'         => esc_html__( 'Post Title', 'frameme' ),
						'comment_count' => esc_html__( 'Comment Count', 'frameme' ),
					),
					'label'   => esc_html__( 'Post sorted', 'frameme' ),
				),
				'post_number' => array(
					'type'       => 'stepper',
					'value'      => '5',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Posts number', 'frameme' ),
				),
				'post_offset' => array(
					'type'       => 'stepper',
					'value'      => '0',
				...
				'columns_number' => array(
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Columns number', 'frameme' ),
				),
				'items_padding' => array(
					'type'       => 'stepper',
					'value'      => '5',
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'frameme_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'       => apply_filters( 'frameme_order_image_grid_widget', 'DESC', $instance ), // ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				$grid_class_array = apply_filters( 'frameme_image_grid_widget_grid_class', array(
				...
				echo apply_filters(
					'frameme_image_grid_widget_before',
				...
				foreach ( $posts as $post ) {
					...
					$image = $this->utility->media->get_image( array(
						'size'              => 'frameme-thumb-m',
						'mobile_size'       => 'frameme-thumb-m',
				...
				echo apply_filters( 'frameme_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'frameme_register_image_grid_widget' );
	/**
	 * Register image grid widget.
	 */
	function frameme_register_image_grid_widget() {
		register_widget( 'FrameMe_Image_Grid_Widget' );
	}
}
