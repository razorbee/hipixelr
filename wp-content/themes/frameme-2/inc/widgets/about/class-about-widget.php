...
if ( ! class_exists( 'FrameMe_About_Widget' ) ) {

	/**
	 * Class FrameMe_About_Widget.
	 */
	class FrameMe_About_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display an information about your site.', 'frameme' );
			$this->widget_id          = 'frameme_widget_about';
			$this->widget_name        = esc_html__( 'About FrameMe', 'frameme' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => '',
				...
				'enable_social' => array(
					...
					'options' => array(
						'enable_social' => esc_html__( 'Enable Social Buttons', 'frameme' ),
					),
				),
				'enable_tagline' => array(
					...
					'options' => array(
						'enable_tagline' => esc_html__( 'Enable Tagline:', 'frameme' ),
					),
				),
				'content'  => array(
					'type'              => 'textarea',
					'placeholder'       => esc_html__( 'Text or HTML', 'frameme' ),
					'value'             => '',
		...
		public function get_social_nav( $wrapper ) {
			...
			if ( is_array( $social_enabled ) && 'true' === $social_enabled['enable_social'] ) {
				$content = sprintf( $wrapper, frameme_get_social_list( 'widget' ) );
			}
		...
		public function widget( $args, $instance ) {
			...
			$src       = wp_get_attachment_image_src( $media_id, apply_filters( 'frameme_about_widget_image_size', 'medium' ) );
			...
			if ( $tagline_enabled ) {
				$format   = apply_filters( 'frameme_about_widget_tagline_format', '<p>%s</p>', $this->settings, $this->args );
...
add_action( 'widgets_init', 'frameme_register_about_widget' );
/**
 * Register about widget.
 */
function frameme_register_about_widget() {
	register_widget( 'FrameMe_About_Widget' );
}
