...
if ( ! class_exists( 'FrameMe_Featured_Posts_Block_Widget' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class FrameMe_Featured_Posts_Block_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Featured Posts Block', 'frameme' );
			$this->widget_description = esc_html__( 'This widget displays latest posts', 'frameme' );
			$this->widget_id          = 'frameme_widget_featured_posts_block';
			...
			foreach( $layouts as $id => $layout ) {
				$layout_options[ $id ] = array(
					'label'   => $layout['name'],
					'img_src' => FRAMEME_THEME_URI . '/assets/images/admin/widgets/featured-posts-block/' . $id . '.svg',
				);
			}
			...
			$this->settings = array(
				...
				'layout' => array(
					...
					'label'   => esc_html__( 'Layout', 'frameme' ),
					'options' => $layout_options,
				),
				'posts_ids' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Posts IDs (Optional)', 'frameme' ),
				),
				'checkboxes'     => array(
					'type'  => 'checkbox',
					'label' => esc_html__( 'Post Meta', 'frameme' ),
					'value' => array(
						'title'      => 'true',
						'excerpt'    => 'true',
						'categories' => 'true',
					...
					'options' => array(
						'title'      => esc_html__( 'Show title', 'frameme' ),
						'excerpt'    => esc_html__( 'Show excerpt', 'frameme' ),
						'categories' => esc_html__( 'Show categories', 'frameme' ),
						'tags'       => esc_html__( 'Show tags', 'frameme' ),
						'author'     => esc_html__( 'Show author', 'frameme' ),
						'date'       => esc_html__( 'Show date', 'frameme' ),
					),
				),
				'title_length' => array(
					'type'      => 'stepper',
					'value'     => 12,
					'min_value' => 1,
					'label'     => esc_html__( 'Title length (chars)', 'frameme' ),
				),
				'excerpt_length' => array(
					'type'      => 'stepper',
					'value'     => 15,
					'min_value' => 1,
					'label'     => esc_html__( 'Excerpt length (words)', 'frameme' ),
				),
			);
		...
		public function render_layout( $options = array() ) {
			...
			$item_template = locate_template( 'inc/widgets/featured-posts-block/views/item.php',false,false );
			 ...
			 */
			$query = apply_filters( 'frameme_featured_posts_block_query', $query, $this->instance );
		...
		public function get_layouts() {
			return array(
				'layout-1' => array(
					'name'   => esc_html__( 'Type #1', 'frameme' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-2' => array(
					'name'   => esc_html__( 'Type #2', 'frameme' ),
					'amount' => 5,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-3' => array(
					'name'   => esc_html__( 'Type #3', 'frameme' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-4' => array(
					'name'   => esc_html__( 'Type #4', 'frameme' ),
					'amount' => 4,
					'markup' => array(
						array(
							'type'  => 'item',
				...
				'layout-5' => array(
					'name'   => esc_html__( 'Type #5', 'frameme' ),
					'amount' => 3,
					'markup' => array(
						array(
							'type'  => 'item',
	...
	add_action( 'widgets_init', 'frameme_register_featured_posts_block_widget' );

	if ( false === function_exists( 'frameme_register_featured_posts_block_widget' ) ) {
		/**
		 * Register featured posts block widget.
		 */
		function frameme_register_featured_posts_block_widget() {
			register_widget( 'FrameMe_Featured_Posts_Block_Widget' );
		}
	}
}
