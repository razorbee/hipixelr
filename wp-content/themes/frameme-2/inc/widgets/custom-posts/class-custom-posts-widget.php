...
if ( ! class_exists( 'FrameMe_Custom_Posts_Widget' ) ) {

	/**
	 * Class FrameMe_Custom_Posts_Widget.
	 */
	class FrameMe_Custom_Posts_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Custom Posts', 'frameme' );
			$this->widget_description = esc_html__( 'Display custom posts your site.', 'frameme' );
			$this->widget_id          = apply_filters( 'frameme_custom_posts_widget_ID', 'widget-custom-posts' );
			$this->widget_cssclass    = apply_filters( 'frameme_custom_posts_widget_cssclass', 'widget-custom-posts custom-posts' );
			$this->utility            = frameme_utility()->utility;
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Custom Posts', 'frameme' ),
				...
				'terms_type' => array(
					...
					'options' => array(
						...
						'post_format'   => array(
							'label' => esc_html__( 'Post Format', 'frameme' ),
				...
				'post_format' => array(
					...
					'options'          => false,
					'label'            => esc_html__( 'Select post format', 'frameme' ),
					'multiple'         => true,
					'placeholder'      => esc_html__( 'Select post format', 'frameme' ),
				...
				'post_offset' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Offset post', 'frameme' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					'value'      => '10',
				...
				'excerpt_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'frameme' ),
				),
				'meta_data' => array(
					'type'    => 'checkbox',
					'value'   => array(
					...
					'options' => array(
						...
						'comment_count' => esc_html__( 'Comment count', 'frameme' ),
						'category'      => esc_html__( 'Category', 'frameme' ),
						'post_tag'      => esc_html__( 'Tag', 'frameme' ),
						'more_button'   => esc_html__( 'More Button', 'frameme' ),
					),
				...
				'button_text' => array(
					...
					'label' => esc_html__( 'Post read more button label', 'frameme' ),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			$footer_columns = get_theme_mod( 'footer_widget_columns', frameme_theme()->customizer->get_default( 'footer_widget_columns' ) );
			...
			if ( $posts ) {
				...
				foreach ( $posts as $post ) {
					...
					if (
						...
						$image_size = 'frameme-thumb-480-271';
					}
	...
	add_action( 'widgets_init', 'frameme_register_custom_posts_widget' );
	/**
	 * Register custom-posts widget.
	 */
	function frameme_register_custom_posts_widget() {
		register_widget( 'FrameMe_Custom_Posts_Widget' );
	}
}
