 ...
 */
add_filter( 'template_include', array( 'FrameMe_Wrapping', 'wrap' ), 99 );
add_filter( 'frameme_wrap_base', 'frameme_wrap_base_cpts' );
 ...
 */
function frameme_template_path() {
	return FrameMe_Wrapping::$main_template;
}
 ...
 */
function frameme_template_base() {
	return FrameMe_Wrapping::$base;
}

if ( ! class_exists( 'FrameMe_Wrapping' ) ) {
	...
	class FrameMe_Wrapping {
		...
		/**
		 * Initialize new instance of FrameMe_Wrapping class by setting a variable for the $slug (which defaults to base)
		...
		public function __toString() {
			$this->templates = apply_filters( 'frameme_wrap_' . $this->slug, $this->templates );
		 ...
		 * and $base as static variables in FrameMe_Wrapping class.
		 *
		 * @since 1.0.0
 ...
 */
function frameme_wrap_base_cpts( $templates ) {
